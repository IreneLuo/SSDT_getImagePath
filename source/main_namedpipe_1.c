...
NTSTATUS PipeConnection(HANDLE *hPipe){
	...
	while (1){
		...
		if (!NT_SUCCESS(ntstatus)){
			...
			DbgPrint("ZwCreateFile Success: handle %x\n",hPipe);
		}
		if (hPipe != 0x0)
			break;
	}
...
int ReceiveOrderFromPipe(HANDLE hPipe){
	NTSTATUS ntstatus;
	INT result = 0;
	UCHAR *pBuffer = NULL;
	IO_STATUS_BLOCK ioStatusBlock = { 0 };
	...
	PVOID ThreadSecurityDescriptor, PVOID Parameter6, PVOID Parameter7, PRTL_USER_PROCESS_PARAMETERS ProcessParameters, PVOID Parameter9, PVOID pProcessUnKnow)
{
	...
	DbgPrint("hPipe %x (%x)\n", &hPipe, hPipe);
	ntstatus = PipeConnection(&hPipe);
	DbgPrint("Handle of pipe %x : %x\n", &hPipe,hPipe);
	if (!NT_SUCCESS(ntstatus)){
		DbgPrint("pipe connection failed: %x\n",ntstatus);
	}
